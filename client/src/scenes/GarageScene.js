...
// Картинка корабля для Гаража: геройский арт (garageKey) если есть, иначе игровой спрайт.
    ...
    for (let i = 0; i < displaySlots; i++) {
      ...
      if (type === 'inventory') {
        ...
        if (CONSUMABLES[item.type]) {
          ...
          // Раньше "нельзя загрузить в слот боеприпасов" (материалы/данж-ресурсы —
          // canLoad всегда false для них, см. комментарий выше) рисовалось тусклой
          // серой рамкой (0x2a3040@0.5) вместо цвета предмета — предмет валиден, просто
          // некуда грузить, а выглядело как нечитаемая/сломанная ячейка (диалог:
          // "сделай более чёткую границу, это биомех ядро"). Цвет предмета теперь
          // всегда, canLoad влияет только на alpha (лёгкий акцент "сюда можно кликнуть").
          const box = this.add.rectangle(sx, sy, SZ, SZ, 0x0a1a2a, 0.95).setOrigin(0, 0)
            .setStrokeStyle(2, hexC, canLoad ? 0.8 : 0.65);
          let iconEl;
  ...
  renderPerksTab() {
    ...
    cy += 20;
    this.add.text(cx, cy, `макс база: ${perkMaxBase(pDef)}`, this.F('12px', '#2a5a6a')).setOrigin(0.5, 0);
    cy += 16;
    this.add.text(cx, cy, `макс ×: ${perkMaxUpgraded(pDef)}`, this.F('12px', '#4dd0e1')).setOrigin(0.5, 0);
    cy += 20;
  ...
  _perkRerollBtn(detX, cy, detW, detH, item, slotIdx, gs) {
    ...
    this.add.text(cx, ry + 8,
      '🔄 Реролл перка',
      this.O('13px', canReroll ? '#bb88dd' : '#334455')).setOrigin(0.5, 0);
    this.add.text(cx, ry + 26,
      canReroll ? `${rerollCost} ⭐  ·  попытка ${rerollN + 1}` : `${rerollCost} ⭐  ·  недостаточно звёзд`,
      this.F('12px', canReroll ? '#ffcc44' : '#2a3040')).setOrigin(0.5, 0);
    this.add.text(cx, ry + 43,
      'Счётчик попыток сбрасывается в 00:00 UTC',
      this.F('10px', '#2a4a60')).setOrigin(0.5, 0);
    if (canReroll) {
