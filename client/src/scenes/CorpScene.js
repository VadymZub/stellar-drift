...
export default class CorpScene extends Phaser.Scene {
  ...
  _drawBounty(px, cy, pw, gs) {
    ...
    const hint = this.add.text(px + pw / 2, cy + 28,
      'Убийца минимум на 3 уровня выше жертвы попадает в розыск (жертва сама решает). Убей разыскиваемого в PvP — получишь ×3 честь + 20 ⭐ (делится по вкладу урона). 10+ жертв на счету — «крупная дичь», награда ×2.',
