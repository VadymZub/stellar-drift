...
// ?devMode=1 был угадываемым по конвенции (диалог: "нужен двойной переключатель,
// мне дев мод ещё пригодится" — риск, что случайный посетитель GameJolt наткнётся
// на публичной веб-версии, набрав это в адресной строке). Секретный токен вместо
// булева флага поднимает планку с "любой знает конвенцию" до "нужно знать именно
// эту строку" — это НЕ криптографическая защита (проект без сборки/минификации,
// весь JS читаем как есть, само значение ниже видно в исходнике), просто убирает
// тривиальное угадывание. Настоящая защита данных — на сервере (is_admin,
// валидация PUT /player/state), не в этой строке; см. диалог для контекста.
const DEV_KEY = 'hjovht369jdenw22kzgugtj28d4b';
const isExplicitDevMode = new URLSearchParams(location.search).get('devKey') === DEV_KEY;
