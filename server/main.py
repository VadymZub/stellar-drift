...
# Потолок одновременных подключений на бета-тест. Старое значение 100 было
# подобрано по прошлому нагрузочному тесту, который якобы упирался в SQLite-
# контеншн уже на ~80-90 живых WS — но живой прогон 2026-08-14 против прод-VPS
# показал, что та деградация была на самом деле из-за двух инфраструктурных
# багов, не связанных с SQLite вообще: (1) дефолтный --ws-ping-timeout uvicorn
# (~20с) — event loop не всегда успевал ответить на ping под нагрузкой, и
# сервер сам рвал полностью живые соединения; (2) LimitNOFILE systemd-юнита
# не был задан явно (дефолт 1024) — упирались в лимit файловых дескрипторов
# на подходе к ~140-150. После фикса обоих (systemd unit: --ws-ping-interval/
# timeout 40, LimitNOFILE=65536) 150 живых WS прошли ЧИСТО, 0 ошибок после
# подключения (см. server/loadtest.py). 150 — это ПОЛЬЗОВАТЕЛИ (уникальные
# uid), не сокеты: реконнект того же игрока не съедает слот.
